...
def eval(args):
    ...
    for case_id in tqdm(case_ids, desc="Processing Predictions"):
        ...
        gt_files = sorted(
            [f for f in os.listdir(case_label_dir) if f.endswith(".nii.gz")]
        )
        pred_files = sorted(
            [
                f
                for f in os.listdir(case_pred_dir)
    ...
    print(
        f"Loaded {len(ground_truths)} ground truth masks and {len(processed_predictions)} processed predictions."
    )

    piccai_score = evaluate(y_det=processed_predictions, y_true=ground_truths)
    print(piccai_score)
    ...
    if auroc_match and ap_match:
        ...
        wandb.log({"AUROC": auroc, "AP": ap, "Overall Score": mean_score})
...
class ProcessPredictions(T.MapTransform):
    ...
    def process_predictions(self, pred):
        ...
        processed_pred = np.expand_dims(
            processed_pred, axis=0
        )  # Add an extra dimension for consistency (as a 3D volume)
