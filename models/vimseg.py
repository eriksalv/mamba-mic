...
from models.utils import interpolate_pos_embed
from models.mamba.models_mamba import VisionMamba, layer_norm_fn, rms_norm_fn, RMSNorm
...
print("Importing VimSegModel")
print("Importing VisionMambaSeg")

class VimSegModel(pl.LightningModule):
    def __init__(self, vimseg, lr=0.001) -> None:
        super().__init__()
        self.lr = lr
        self.criterion = DiceCELoss(sigmoid=True, squared_pred=True)
        self.dice_metric = DiceHelper(sigmoid=True)
        self.vimseg = VisionMambaSeg(**vimseg)
        self.save_hyperparameters()
        print(f"VimSegModel initialized with vimseg: {vimseg}")

    def forward(self, x):
        return self.vimseg(x)
...
def test_vision_mamba_init():
    try:
        # Attempt to initialize the model
        vimseg = VisionMambaSeg(img_size=512, patch_size=16, in_chans=3, num_classes=3)
        print("VisionMambaSeg initialized successfully")
    except Exception as e:
        print("Failed to initialize VisionMambaSeg:", e)

if __name__ == "__main__":
    test_vision_mamba_init()